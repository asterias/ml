...
for n in dimensions:
	...
	for i in range(sample_size):
		for j in range(sample_size):
			if i != j:
				if (dist.euclidean(x_matrix[i],x_matrix[j]) < temp_min[i]):
					min_array[i] = dist.euclidean(x_matrix[i],x_matrix[j])
					temp_min[i] = min_array[i]
				if (dist.euclidean(x_matrix[i],x_matrix[j]) > temp_max[i]):
					max_array[i] = dist.euclidean(x_matrix[i],x_matrix[j])
